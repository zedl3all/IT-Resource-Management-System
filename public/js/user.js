...
document.addEventListener("DOMContentLoaded", function () {
  ...
  function setupEventListeners() {
    ...
    // Booking filtering
    document
      .getElementById("booking-filter")
      ?.addEventListener("change", filterBookings);

    // Add maintenance button
    document
      .getElementById("add-maintenance-btn")
    ...
    // Form submissions
    document
      .getElementById("room-booking-form")
      ?.addEventListener("submit", handleRoomBooking);
    document
      .getElementById("equipment-booking-form")
      ?.addEventListener("submit", handleEquipmentBooking);
    document
      .getElementById("maintenance-form")
      ?.addEventListener("submit", handleMaintenanceRequest);
  ...
  function handleButtonClicks(e) {
    ...
    if (
      ...
      e.target.closest("#available-rooms-table")
    ) {
    ...
    // Equipment booking button
      ...
      e.target.closest(".btn-book") &&
      e.target.closest("#available-equipment-table")
    ) {
  ...
  function handleRoomBookingButton(button) {
    ...
    document.getElementById(
      "selected-room-name"
    ).textContent = `${roomName} (${roomId})`;
    document.getElementById(
      "selected-room-details"
    ).textContent = `${roomDescription} - ความจุ: ${roomCapacity} คน`;
  ...
  function handleEquipmentBookingButton(button) {
    ...
    ).textContent = `${equipmentName} (${equipmentId})`;
    document.getElementById(
      "selected-equipment-details"
    ).textContent = `ประเภท: ${equipmentType}`;
  ...
  function handleRoomsUpdate() {
    const active = document.querySelector(
      '.menu-item.active[data-section="rooms"]'
    );
  ...
  function handleEquipmentsUpdate() {
    const active = document.querySelector(
      '.menu-item.active[data-section="equipment"]'
    );
  ...
  function loadSectionData(section) {
    switch (section) {
      case "bookings":
        loadMyBookings(); // โหลดทั้งสองประเภท
        break;
      case "rooms":
        loadAvailableRooms();
        break;
  ...
  // เพิ่มฟังก์ชันสำหรับแสดงข้อผิดพลาดแยกตามประเภท
  ...
  function displayEquipmentBookingsError() {
    const container = document.getElementById(
      "my-equipment-bookings-container"
    );
    if (container) {
      container.innerHTML = createEmptyStateHTML(
        "exclamation-circle",
      ...
      .then((data) => {
        ...
        data.rooms.forEach((room) => {
          ...
          row.innerHTML = `
                        ...
                        <td><span class="status ${statusInfo.class}">${statusInfo.text
            }</span></td>
                        <td class="actions">
    ...
    apiGet("/api/equipments")
      .then((data) => {
        const tbody = document.querySelector(
          "#available-equipment-table tbody"
        );
        tbody.innerHTML = "";
  ...
  function loadMyMaintenanceRequests() {
    apiGet("/api/user/maintenance-requests")
      ...
      .catch((error) =>
        handleApiError(error, "Error loading maintenance requests")
      );
  }
  ...
  function displayRoomBookings(roomBookings) {
    ...
    if (roomBookings.length > 0) {
      ...
      container.innerHTML = createEmptyStateHTML(
        "door-open",
        "ยังไม่มีการจองห้อง",
        "เลือกจองห้องเพื่อดูรายการที่นี่"
      );
    }
  }
  ...
  function displayEquipmentBookings(equipmentBookings) {
    ...
    if (equipmentBookings.length > 0) {
      ...
      container.innerHTML = createEmptyStateHTML(
        "laptop",
        "ยังไม่มีการจองอุปกรณ์",
        "เลือกจองอุปกรณ์เพื่อดูรายการที่นี่"
      );
    }
  }
            ...
            <div class="booking-body">
                <div class="booking-details">
                    <div class="booking-detail">
                        ...
                        <span>วันที่: <b>${formatDate(booking.booking_date || booking.start_time || booking.borrow_DT)}</b></span>
                    </div>
                    <div class="booking-detail">
                        <i class="fas fa-clock"></i>
                        <span>วันที่: <b>${formatDate(booking.start_time || booking.borrow_DT)}</b> เวลา: <b>${formatTime(booking.start_time || booking.borrow_DT)}</b> ถึง
                              วันที่: <b>${formatDate(booking.end_time || booking.return_DT)}</b> เวลา: <b>${formatTime(booking.end_time || booking.return_DT)}</b>
                        </span>
                    </div>
                    <div class="booking-detail">
  ...
  function filterBookings() {
    ...
    const roomBookings = document.querySelectorAll(
      "#my-room-bookings-container .booking-item"
    );
    const equipmentBookings = document.querySelectorAll(
      "#my-equipment-bookings-container .booking-item"
    );
    ...
    checkEmptyState(
      "my-room-bookings-container",
      "ไม่พบการจองห้องที่ตรงกับเงื่อนไข",
      "door-open"
    );
    checkEmptyState(
      "my-equipment-bookings-container",
      "ไม่พบการจองอุปกรณ์ที่ตรงกับเงื่อนไข",
      "laptop"
    );
  }
  ...
  function checkEmptyState(containerId, message, iconName) {
    ...
    const visibleItems = Array.from(
      container.querySelectorAll(".booking-item")
    ).filter((item) => item.style.display !== "none");

    // If we have booking items but none are visible after filtering
    if (
      visibleItems.length === 0 &&
      container.querySelectorAll(".booking-item").length > 0
    ) {
  ...
  function updateBookingStats(bookings) {
    const stats = {
      ...
      active: bookings.filter(
        (b) => getBookingStatus(b).class === STATUS_CLASS_MAP.ACTIVE
      ).length,
      upcoming: bookings.filter(
        (b) => getBookingStatus(b).class === STATUS_CLASS_MAP.UPCOMING
      ).length,
    };

    const statCards = document.querySelectorAll(
      "#bookings .stat-card .stat-value"
    );
  ...
  function getStatusClassFromElement(element) {
    if (element.classList.contains(STATUS_CLASS_MAP.ACTIVE))
      return STATUS_CLASS_MAP.ACTIVE;
    if (element.classList.contains(STATUS_CLASS_MAP.UPCOMING))
      return STATUS_CLASS_MAP.UPCOMING;
  ...
  function getRoomStatusInfo(status) {
    ...
    return {
      class:
        statusCode === 1
          ? STATUS_CLASS_MAP.AVAILABLE
          : statusCode === 0
            ? STATUS_CLASS_MAP.BOOKED
  ...
  function formatTime(timeString) {
    ...
    // กรณีเป็นเวลาในรูปแบบ ISO หรือเป็น timestamp
